...
var endecode = {
	...
	base64Encode : function (input) {
		...
		input = endecode._utf8_encode(input);
	...
	base64Decode : function (input) {
		...
		output = endecode._utf8_decode(output);
	...
	replaceAll : function(tmpStr, str1, str2) {
		while(1) {
			if (tmpStr.indexOf(str1) != -1) {
	...
	unicodeEncode : function(val) {
		return escape(this.replaceAll(val, '\\', '%'));
	},
	
	unicodeDecode : function(val) {
		return unescape(this.replaceAll(val, '\\', '%'));
	}
}
...
function convert() {
	...
	$('#urlEncode').val(endecode.urlEncode(val));
